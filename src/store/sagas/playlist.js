...
import { removeItemFromStorage, persistItemInStorage, getItemFromStorage } from '~/utils/AsyncStorageManager'

import { downloadPodcast, stopPodcastDownload, removePodcastFromLocalStorage } from './localPodcastsManager'

import CONSTANTS from '~/utils/CONSTANTS'

export function* loadPlaylists() {
	try {
		const playlistsFromStorage = yield getItemFromStorage(CONSTANTS.KEYS.PLAYLIST_STORAGE_KEY, [])

		const playlists = typeof playlistsFromStorage === 'string' ? JSON.parse(playlistsFromStorage) : []
...
export function* createPlaylist({ payload }) {
	try {
		...
		yield persistItemInStorage(CONSTANTS.KEYS.PLAYLIST_STORAGE_KEY, playlistsUpdated)

		yield put(PlaylistCreators.createPlaylistSuccess(playlistsUpdated))
	} catch (err) {
		yield put(PlaylistCreators.createPlaylistFailure('An unexpected error happened. Sorry for that.'))
	}
}
...
function* _handlePersistsPlaylistsUpdated(playlist, podcasts) {
	...
	const playlistsUpdated = playlists.map((playlistFromState) => (playlistFromState.title === playlist.title ? playlistUpdated : playlistFromState))
...
export function* removePlaylist({ payload }) {
	try {
		...
		const playlistsUpdated = playlists.filter((playlist) => playlist.title !== playlistToRemove.title)
	...
	try {
		...
		if (playlist.isAvailableOffline) {
			...
			if (!isPodcastAlreadyDownloaded) {
				playlistUpdated = {
					...playlistUpdated,
					downloads: [podcast._id, ...playlistUpdated.downloads],
				}
		...
		const playlistsUpdated = yield call(_handlePersistsPlaylistsUpdated, playlistUpdated, podcastsUpdated)

		yield put(PlaylistCreators.addPodcastSuccess(playlistsUpdated))
	} catch (err) {
		yield put(PlaylistCreators.createPlaylistFailure('An unexpected error happened while try to add podcast on playlist. Sorry for that.'))
	}
}
...
export function* removePodcast({ payload }) {
	try {
		...
		if (playlist.isAvailableOffline) {
			...
			const isPodcastDownloadedByPlaylist = playlist.downloads.some((podcastId) => podcastId === podcast._id)
		...
		const podcastsUpdated = playlist.podcasts.filter((podcastInPlaylist, index) => index !== podcastIndex)

		const playlistsUpdated = yield call(_handlePersistsPlaylistsUpdated, playlist, podcastsUpdated)
...
export function* getPlaylist({ payload }) {
	try {
		...
		const playlistSelected = playlists.find((playlistInStore) => playlistInStore.title === title)

		const podcasts = playlistSelected.podcasts.map((podcast) => ({
			...podcast,
			isDownloaded: podcastsDownloaded.some((podcastDownloaded) => podcastDownloaded._id === podcast._id),
		}))
...
function* _setPlaylistToAvailableOffline(playlistSelected) {
	try {
		...
		const playlistUpdated = {
			...
			downloads: podcastsToDownload.map((podcast) => podcast._id),
		}

		yield call(_handlePersistsPlaylistsUpdated, playlistUpdated, playlistSelected.podcasts)

		const playlistsUpdated = playlist.playlists.map((playlist) => (playlist.title === playlistUpdated.title ? playlistUpdated : playlist))

		yield put(PlaylistCreators.setOfflineAvailabilitySuccess(playlistUpdated, playlistsUpdated))

		yield all(podcastsToDownload.map((podcast) => call(downloadPodcast, podcast)))
	} catch (err) {
		throw err
	}
}
...
function* _setPlaylistToUnvailableOffline(playlistSelected) {
	try {
		...
		const jobsToCancel = downloadingList.filter((downloadingItem) => {
			const shouldCancelPodcatDownload = playlistSelected.downloads.some((podcastId) => downloadingItem._id === podcastId)
			return shouldCancelPodcatDownload
		})

		const podcastsAlreadyDownloadedByPlaylistSelected = playlistSelected.downloads.filter((podcast) => {
			const isPodcastAlreadyDownloaded = podcastsDownloaded.some((podcastDownloaded) => podcastsDownloaded._id === podcast._id)
			return isPodcastAlreadyDownloaded
		})
