...
const ProgramTitle = styled(Text).attrs({
	...
	font-family: ${Platform.OS === 'android' ? 'CircularStd-Medium' : 'CircularStd-Black'};
...
const ProgramCategoriesListItem = ({ program, isLastIndex, onPressItem }) => (
	<Wrapper isLastIndex={isLastIndex} onPress={() => onPressItem(program.id)}>
		<SourceImage uri={program.imageUrl} />
		<DarkLayer>
			<AuthorInfoWrapper>
				<AuthorInfo imageURL={program.publisher.imageUrl} numberOfLines={2} textColor="white" name={program.publisher.title} />
			</AuthorInfoWrapper>
			<View>
