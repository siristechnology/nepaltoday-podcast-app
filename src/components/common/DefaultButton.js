...
const ButtonWrapper = styled(TouchableOpacity)`
	...
	border: ${({ translucent, theme }) => (translucent ? theme.colors.textColor : 'transparent')} solid 1.5px;
	border-radius: 4.5px;
`

const Title = styled(Text)`
	color: ${({ translucent, theme }) => (translucent ? theme.colors.textColor : theme.colors.white)};
	font-size: ${({ theme, size }) => (size === 'large' ? theme.metrics.largeSize : theme.metrics.mediumSize * 1.2)}px;
...
const ListenNowButton = ({ translucent, onPress, size, text }: Props): Object => (
