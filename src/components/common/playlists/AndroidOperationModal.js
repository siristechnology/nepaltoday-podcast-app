...
const AndroidOperationModal = ({ onTypePlaylistTitle, playlistTitle, toggleModal, mainAction, error, mode }: Props): Object => (
	<Modal onRequestClose={toggleModal} animationType="slide" hardwareAccelerated transparent>
		<AndroidAlertContainer>
			<CardContainer>
				<View>
					<CardTitle>{`${mode} Playlist`}</CardTitle>
					{!!error && <ErrorMessage>{error}</ErrorMessage>}
					<Input onChangeText={(text) => onTypePlaylistTitle(text)} value={playlistTitle} />
				</View>
