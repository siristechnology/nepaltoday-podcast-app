...
const PlaylistListItem = ({ onRemovePlaylist, numberOfPodcasts, onEditPlaylist, isDownloaded, onPress, images, title, theme }: Props): Object => (
	<Wrapper>
		<Swipeout
			autoClose
			...
			right={[
				{
					component: <SwipeOutButton color={appStyles.colors.primaryColor} icon="trash-can-outline" />,
					onPress: onRemovePlaylist,
					type: 'delete',
				},
				{
					component: <SwipeOutButton color={appStyles.colors.subTextWhite} icon="pencil" />,
			...
			<ContentWrapper onPress={onPress}>
				...
				<TextWrapper>
					...
					<BottomContentWrapper>
						<NumberPodcasts>{`${numberOfPodcasts} ${numberOfPodcasts === 1 ? 'Podcast' : 'Podcasts'}`}</NumberPodcasts>
						<Icon
							color={isDownloaded ? appStyles.colors.primaryColor : appStyles.colors.subTextWhite}
