...
class Playlists extends Component<Props, State> {
	...
	onTogglePlaylistOperationModal = (modalMode: string, playlistTitle: string, index: number): void => {
	...
	checkPlaylistTitleIsAlreadyInUse = (title: string): boolean => {
		...
		const isTitleAlreadyInUse = playlists.some((playlist) => playlist.title.toUpperCase() === title.toUpperCase())
	...
	render() {
		const { isPlaylistOperationModalOpen, isPlaylistTitleAlreadyInUse, playlistTitle, modalMode } = this.state
			...
			<Wrapper>
				...
				{isPlaylistOperationModalOpen && (
					<PlaylistOperationModal
						...
						mainAction={isModalCreationMode ? this.onCreatePlaylist : this.onEditPlaylist}
