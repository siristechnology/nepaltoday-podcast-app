...
const iOSModalAlert = (mainAction: Function, title: string, error, defaultText: string, toggleModal: Function, mode: string): void => {
	AlertIOS.prompt(
		title,
		error,
		[
...
const PlaylistOperationModal = ({ onTypePlaylistTitle, playlistTitle, toggleModal, mainAction, hasError, mode }: Props): Object => {
	const title = `${mode} Playlist`
	const error = hasError ? 'This title is already in use by other Playlist.' : ''
