...
const renderCenterButton = (paused: boolean, pause: Function, play: Function): Object => {
	const { onPress, iconName } = paused ? { onPress: play, iconName: 'play' } : { onPress: pause, iconName: 'pause' }

	return (
		<PlayOutterCircle>
			<PlayInnerCircleButton onPress={onPress}>
				<Icon size={appStyles.metrics.getWidthFromDP('9%')} color={appStyles.colors.white} name={iconName} />
