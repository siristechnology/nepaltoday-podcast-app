...
import { TouchableOpacity, Animated, FlatList, Platform, Text, View } from 'react-native'
...
class CustomTab extends Component<Props, State> {
	...
	setMarkerPosition = (newIndexSelected: number): any => {
		...
		const shouldNotRenderMarker =
			itemSelectedIndex > 0 && itemSelectedIndex < data.length - 1 && newIndexSelected > 0 && newIndexSelected < data.length - 1
				...
				<FlatList
					...
					renderItem={({ item, index }) => (
						<Cell
							...
							width={cellWidth}
						>
							<OptionText color={itemSelectedIndex === index ? activeTextColor : inactiveTextoColor}>{item.title}</OptionText>
						</Cell>
					)}
				/>
			</ListWrapper>
	...
	renderMarker = (markerColor: string) => {
		...
		return (
			...
			>
				<Marker currentIndex={itemSelectedIndex} color={markerColor} width={cellWidth} />
			</MarkerWrapper>
		)
	}
	...
	render() {
		...
		const { inactiveTextoColor, activeTextColor, markerColor, cellColor } = this.getThemeColors(theme)
