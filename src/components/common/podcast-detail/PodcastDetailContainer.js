...
class PodcastDetail extends Component<Props, State> {
	...
	onPressDownloadButton = (isPodcastDownloaded: boolean, podcast: Object): void => {
		...
		if (isPodcastDownloaded) {
			const isPodcastDownloadedByPlaylist = this.checkPodcastAvailableOfflineWithPlaylist(podcast)

			type = isPodcastDownloadedByPlaylist ? TYPES.REMOVE_DOWNLOADED_PODCAST_BY_PLAYLIST : TYPES.REMOVE_DOWNLOADED_PODCAST
