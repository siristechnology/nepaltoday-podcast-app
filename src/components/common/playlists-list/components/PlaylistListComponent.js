...
import { TouchableOpacity, FlatList, Platform, Modal, View, Text } from 'react-native'
...
const Container = styled(View)`
	width: 100%;
	height: 100%;
	padding-top: ${({ theme }) => (Platform.OS === 'android' ? theme.metrics.mediumSize : theme.metrics.extraLargeSize)}px;
...
const PlaylistListComponent = ({ onPressPlaylistListItem, createPlaylist, onToggleModal, playlists, theme }: Props): Object => (
	<Modal onRequestClose={onToggleModal} animationType="slide" hardwareAccelerated transparent={false}>
		<Container>
			<HeaderWrapper>
			...
			{playlists.length === 0 && <CreatePlaylistButton createPlaylist={createPlaylist} />}
		</Container>
	</Modal>
)
