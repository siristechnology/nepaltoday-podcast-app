...
class PlaylistListContainer extends Component<Props, {}> {
	...
	onPressPlaylistListItem = (playlist: Playlist): void => {
		const isPodcastAlreadyInPlaylist = this.checkIsPodcastAlreadyInPlaylist(playlist)

		const isPodcastAlreadyDownloaded = this.checkIsPodcastAlreadyDownloaded(playlist)

		if (playlist.isAvailableOffline && !isPodcastAlreadyInPlaylist && !isPodcastAlreadyDownloaded) {
			CustomAlert(TYPES.ADD_UNDOWNLOADED_PODCAST_PLAYLIST_AVAILABLE_OFFLINE, () => this.onAddPodcast(playlist))
...
export default connect(mapStateToProps, mapDispatchToProps)(PlaylistListContainer)
