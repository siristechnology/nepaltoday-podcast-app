...
const PlaylistCompositionImages = ({ images, size }: Props): Object => (
	<Wrapper size={size}>
		{images.length === 0 && (
			<PodcastIconWrapper>
				<Icon color={appStyles.colors.primaryColor} name="headphones" size={28} />
			</PodcastIconWrapper>
		)}
		{images.length > 0 && images.map((url, index) => <PodcastImage index={index} size={size} key={`${url}${index}`} uri={url} />)}
	</Wrapper>
)
